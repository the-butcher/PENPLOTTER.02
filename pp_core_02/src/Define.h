...
#define COMMAND_BUFF_VALS_SIZE 20
...
typedef struct {
    ...
    int64_t r;         // r coord with respect to r home (unit?)
} coord_planxy_i64_t;  // 32 bytes
...
typedef struct {
    ...
    float r;           // r coord with respect to z home (unit?)
} coord_planxy_f___t;  // 16 bytes
...
typedef struct {
    ...
    int64_t r;         // exit r coord with respect to r home (unit?)
    int64_t vi;        // entry speed (µm/s)
    int64_t vo;        // exit speed (µm/s)
} block_planxy_i64_t;  // 48 bytes
...
typedef struct {
    ...
    float r;           // exit r coord with respect to r home (unit?)
    float vi;          // entry speed (mm/s)
    float vo;          // exit speed (mm/s)
} block_planxy_f___t;  // 24 bytes
...
typedef struct {
    ...
    int64_t r;         // rot motor coord (microstep settings as of motor-R constants)
